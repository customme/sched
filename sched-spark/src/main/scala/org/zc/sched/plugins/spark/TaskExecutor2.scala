...
abstract class TaskExecutor2(task: Task) extends Serializable with Log {
...
object TaskExecutor2 extends Log {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (task.isDefined) {
      Class.forName(appClass).getConstructors.head.newInstance(task.get).asInstanceOf[TaskExecutor2].run
    } else {
  ...
  def getTask(taskId: Int, runTime: String) = {
    val sql = "SELECT a.name, a.task_cycle, IF(b.run_time = a.first_time, 1, 0) is_first, b.redo_flag, b.extra_param" +
  ...
  def getTaskExt(task: Task) = {
    ...
    val sql = "SELECT prop_name, prop_value FROM t_task_ext a WHERE a.task_id = ? AND a.version_no = (SELECT MAX(version_no) FROM t_task_ext b WHERE a.task_id = b.task_id)"
    ps = conn.prepareStatement(sql)
    ps.setInt(1, task.taskId)
